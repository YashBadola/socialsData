...
def test_load_soren_kierkegaard_dataset():
    ...
    expected_sources = {
        ...
        "in_vino_veritas.txt",
        "preparation_for_a_christian_life.txt",
        "the_present_moment.txt"
    }

    # Verify that we have at least one of the expected sources
    assert len(sources.intersection(expected_sources)) > 0, f"Found unexpected sources: {sources}"

    # Specific keywords we expect in Kierkegaard's text
    keywords = ["Abraham", "faith", "love", "God", "Christian"]
