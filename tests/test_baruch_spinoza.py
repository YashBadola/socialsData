...
def test_baruch_spinoza_metadata():
    ...
    # Check that "Ethics" is in one of the source titles
    titles = [s["title"] for s in metadata["sources"]]
    assert any("Ethics" in t for t in titles)
...
def test_baruch_spinoza_data_loaded():
    ...
    # Check for Spinozistic terminology
    # Since we can't guarantee order, we check if at least one entry contains "God" or "Substance"
    found_term = False
    for entry in dataset:
        if "God" in entry["text"] or "Substance" in entry["text"]:
            found_term = True
            break
    assert found_term
