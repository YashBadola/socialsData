...
def process(personality_id, skip_qa):
    ...
    click.echo(f"Done. Data saved to {personality_dir}/processed/data.jsonl and Database.")
    ...
    if qa_file.exists() and qa_file.stat().st_size > 0:
        click.echo(f"Done. Q&A data saved to {qa_file} and Database.")
...
@main.command()
def migrate():
    """Migrate existing personalities from file system to database."""
    manager = PersonalityManager()
    click.echo("Migrating personalities...")
    manager.migrate_from_files()
