...
def test_niccolo_machiavelli_data_loaded():
    ...
    # The source filename in the processed data should match the raw filename
    assert sample["source"] == "the_prince.txt"

    assert isinstance(sample["text"], str)
    assert len(sample["text"]) > 100
    # "Prince" usually appears in the text
    assert "Prince" in sample["text"] or "Machiavelli" in sample["text"]
