...
def test_seneca_data_loaded():
    ...
    assert sample["source"] in ["on_the_shortness_of_life.txt", "letters_to_lucilius.txt", "on_anger.txt"]

    # Check for keywords
    all_text = " ".join([d["text"] for d in dataset])
    assert "short time" in all_text or "Lucilius" in all_text or "Anger" in all_text
