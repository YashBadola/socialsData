...
def test_load_aristotle_dataset():
    ...
    # We expect exactly these two sources since we processed them
    assert sources == expected_sources, f"Expected {expected_sources}, found {sources}"

    # Specific keywords we expect in Aristotle's text
    keywords = ["virtue", "happiness", "state", "citizen", "mean"]
