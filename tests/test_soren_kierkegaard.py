...
def test_load_kierkegaard_dataset():
    ...
    # We expect at least these sources to be present
    assert expected_sources.intersection(sources), f"Expected sources not found. Found: {sources}"

    # Specific keywords we expect in Kierkegaard's text
    keywords = ["Abraham", "ethical", "paradox", "faith", "boredom", "poet"]
