...
def test_load_kierkegaard_dataset():
    ...
    # We expect our test files to be there
    assert sources.issubset(expected_sources) and len(sources) > 0, f"Found unexpected sources: {sources}"

    # Specific keywords we expect in Kierkegaard's text
    keywords = ["Abraham", "faith", "poet", "anguish", "truth"]
