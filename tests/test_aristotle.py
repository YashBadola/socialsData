...
def test_load_aristotle_dataset():
    ...
    expected_sources = {"nicomachean_ethics.txt"}
    ...
    keywords = ["virtue", "happiness", "mean", "habit", "politics", "eudaimonia"]
    found_keywords = False
    for item in dataset:
        for keyword in keywords:
            if keyword in item["text"] or keyword.capitalize() in item["text"]:
                found_keywords = True
                break
        if found_keywords:
            break
