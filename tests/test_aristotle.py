...
def test_load_aristotle_dataset():
    ...
    # Since I just created them, they should all be there.
    assert expected_sources.issubset(sources), f"Missing expected sources. Found: {sources}, Expected subset: {expected_sources}"

    # Specific keywords we expect in Aristotle's text
    keywords = ["virtue", "political", "metaphysics", "nature", "mean", "state", "good"]
