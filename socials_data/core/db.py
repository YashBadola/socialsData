...
class SocialsDatabase:
    ...
    def _init_db(self):
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS sources (
                ...
                UNIQUE(personality_id, title)
            )
        ''')

        # Note: Indexing full text for unique constraint might be heavy,
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS content (
                ...
                UNIQUE(personality_id, source_id, text)
            )
        ''')
        self.conn.commit()
    ...
    def add_source(self, personality_id: str, source_meta: Dict) -> int:
        # Check if exists to return id
        try:
            self.cursor.execute('''
                INSERT INTO sources (personality_id, title, url, type)
                VALUES (?, ?, ?, ?)
            ''', (personality_id, source_meta.get('title'), source_meta.get('url'), source_meta.get('type')))
            self.conn.commit()
            ...
            ''', (personality_id, source_meta.get('title')))
    ...
    def add_content(self, personality_id: str, text: str, content_type: str = "text", source_id: Optional[int] = None, meta: Optional[Dict] = None):
        # Check for existence manually because SQLite UNIQUE(..., NULL, ...) treats NULLs as distinct
        if source_id is None:
            self.cursor.execute('''
                SELECT id FROM content
                WHERE personality_id = ? AND source_id IS NULL AND text = ?
            ''', (personality_id, text))
        else:
            self.cursor.execute('''
                SELECT id FROM content
                WHERE personality_id = ? AND source_id = ? AND text = ?
            ''', (personality_id, source_id, text))
