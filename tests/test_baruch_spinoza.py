...
def test_baruch_spinoza_metadata():
    ...
    assert "Ethics" in [s["title"] for s in metadata["sources"]] or \
           "Ethics (Ethica Ordine Geometrico Demonstrata)" in [s["title"] for s in metadata["sources"]]
