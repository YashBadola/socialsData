...
def test_load_aristotle_dataset():
    ...
    # Also check that we have at least one of the expected sources
    assert len(sources.intersection(expected_sources)) > 0, "Did not find any expected sources"

    # Specific keywords we expect in Aristotle's text
    keywords = ["virtue", "polis", "happiness", "mean", "good"]
