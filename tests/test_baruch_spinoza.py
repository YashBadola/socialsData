...
def test_baruch_spinoza_metadata():
    ...
    assert "Ethics, Demonstrated in Geometrical Order" in [s["title"] for s in metadata["sources"]]
...
def test_baruch_spinoza_data_loaded():
    ...
    # Check for "SUBSTANCE" or "GOD"
    assert "SUBSTANCE" in sample["text"] or "GOD" in sample["text"]
