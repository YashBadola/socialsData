...
def test_load_aristotle_dataset():
    ...
    # Ideally we want to find both sources if the dataset is complete
    assert "nicomachean_ethics.txt" in sources, "Missing Nicomachean Ethics"
    assert "politics.txt" in sources, "Missing Politics"

    # Specific keywords we expect in Aristotle's text
    keywords = ["virtue", "happiness", "state", "citizen", "mean", "political animal"]
