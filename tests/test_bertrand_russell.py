...
def test_dataset_content_bertrand_russell():
    ...
    # Check for keywords
    keywords = ["philosophy", "logic", "mind", "knowledge", "reason"]
...
def test_dataset_sources_bertrand_russell():
    """Test that both sources are present."""
    dataset = load_dataset("bertrand_russell")
    sources = set(item["source"] for item in dataset)

    assert "problems_of_philosophy.txt" in sources
    assert "analysis_of_mind.txt" in sources
...
def test_metadata_bertrand_russell():
    ...
    assert "analytic philosopher" in metadata["system_prompt"]
