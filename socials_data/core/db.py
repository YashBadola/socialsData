...
class Database:
    ...
    def clear_personality_data(self, personality_id):
        self.connect()
        cursor = self.conn.cursor()

        # Get all document IDs for this personality
        ...
        doc_ids_placeholder = ','.join(['?'] * len(doc_ids))
        cursor.execute(f"SELECT id FROM chunks WHERE document_id IN ({doc_ids_placeholder})", doc_ids)
        chunk_ids = [row['id'] for row in cursor.fetchall()]

        # Delete QA pairs
        if chunk_ids:
            chunk_ids_placeholder = ','.join(['?'] * len(chunk_ids))
            cursor.execute(f"DELETE FROM qa_pairs WHERE chunk_id IN ({chunk_ids_placeholder})", chunk_ids)

        # Delete Chunks
        cursor.execute(f"DELETE FROM chunks WHERE document_id IN ({doc_ids_placeholder})", doc_ids)
