...
def test_load_vitruvius_dataset():
    ...
    expected_sources = {"ten_books_on_architecture.txt"}

    assert sources.issubset(expected_sources), f"Found unexpected sources: {sources - expected_sources}"

    # Specific keywords we expect in Vitruvius' text
    keywords = ["architecture", "symmetry", "proportion", "columns", "temple"]
