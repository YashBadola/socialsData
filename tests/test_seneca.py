...
def test_seneca_metadata():
    ...
    assert "Seneca's Morals of a Happy Life, Benefits, Anger and Clemency" in [s["title"] for s in metadata["sources"]]
...
def test_seneca_data_loaded():
    ...
    assert sample["source"] == "seneca_morals.txt"

    # Check for keywords from "Seneca's Morals"
