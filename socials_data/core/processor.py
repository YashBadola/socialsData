...
class TextDataProcessor(DataProcessor):
    ...
    def _chunk_text(self, text, chunk_size=2000):
        ...
        respecting paragraph boundaries (double newlines).
        """
        chunks = []
        current_chunk = []
        current_length = 0

        # Normalize newlines to ensure consistent paragraph splitting
        text = text.replace('\r\n', '\n')

        # Split by double newlines to preserve paragraphs
        # We also treat single newlines as potential split points if needed,
        # but Gutenberg texts usually use double newlines for paragraphs.
        # However, the previous cleaning logic collapsed everything.
        # Let's trust the raw text's structure or standard Gutenberg formatting.
        ...
        for paragraph in paragraphs:
            ...
            # Basic cleanup of the paragraph itself (remove single newlines within it if they are just wrapping)
            # Many Gutenberg texts have fixed line width. We should unwrap them.
