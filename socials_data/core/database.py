...
class DatabaseManager:
    ...
    def list_personalities(self) -> List[Dict[str, Any]]:
        ...
        cursor.execute('''
            SELECT p.*, s.title, s.url, s.type
            FROM personalities p
            LEFT JOIN sources s ON p.id = s.personality_id
        ''')
        ...
        for row in rows:
            p_id = row['id']
            if p_id not in personalities_map:
                personalities_map[p_id] = {
                    'id': row['id'],
                    'name': row['name'],
            ...
            if row['title']: # check if source exists
                personalities_map[p_id]['sources'].append({
                    'title': row['title'],
                    'url': row['url'],
                    'type': row['type']
                })
        ...
        conn.close()
        return list(personalities_map.values())
