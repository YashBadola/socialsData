...
def test_load_aristotle_dataset():
    ...
    # Ensure both sources are present (since we processed both)
    assert "nicomachean_ethics.txt" in sources, "nicomachean_ethics.txt missing from sources"
    assert "politics.txt" in sources, "politics.txt missing from sources"

    # Specific keywords we expect in Aristotle's text
    keywords = ["virtue", "mean", "happiness", "reason", "ethics", "state", "constitution"]
