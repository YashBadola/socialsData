...
def test_load_aristotle_dataset():
    ...
    # Check that sources match exactly (since we processed all of them)
    assert sources == expected_sources, f"Found unexpected sources or missing sources. Found: {sources}, Expected: {expected_sources}"

    # Specific keywords we expect in Aristotle's text
    keywords = ["virtue", "happiness", "polis", "metaphysics", "state", "good"]
    found_keywords = False

    # Check if at least one keyword is found across the dataset
