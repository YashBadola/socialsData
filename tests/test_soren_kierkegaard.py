...
def test_load_kierkegaard_dataset():
    ...
    # Also assert that we found at least one of them (since we just added them, we expect both if processing worked fully)
    assert len(sources) > 0, "No sources found"

    # Specific keywords we expect in Kierkegaard's text
    keywords = ["faith", "paradox", "individual", "boredom", "choice", "Either/Or"]
