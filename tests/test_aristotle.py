...
def test_load_aristotle_dataset():
    ...
    # We check that the sources present are valid.
    assert sources.issubset(expected_sources), f"Found unexpected sources: {sources - expected_sources}"

    # Also check that we found at least one of them
    assert len(sources) > 0, "No sources found in dataset"

    # Specific keywords we expect in Aristotle's text
    keywords = ["virtue", "politics", "tragedy", "happiness", "state", "citizen"]
